...
OUT_ENV="${1:-./accounts_pool/keep_pool.env}"
POOL_SIZE="${2:-1}"
...

{
  ...
  echo "export KEEP_POOL_SIZE=\"$POOL_SIZE\""
} > "$OUT_ENV"

FUND_AMOUNT_NUM="$(echo "$FUND_AMOUNT" | sed 's/[[:space:]]*ether$//I')"

for idx in $(seq 1 "$POOL_SIZE"); do
  echo "[*] creating KEEP pool account $idx/$POOL_SIZE"
  wallet_info="$(make_wallet)"
  addr="$(echo "$wallet_info" | extract_address)"
  key="$(echo "$wallet_info" | extract_private_key)"
  validate_hex_address "$addr" || { echo "[!] invalid KEEP pool address $idx" >&2; exit 1; }
  validate_hex_privkey "$key" || { echo "[!] invalid KEEP pool private key $idx" >&2; exit 1; }
  ...
  validate_tx_hash "$tx_hash" || { echo "[!] failed to fund KEEP pool account $idx: $send_json" >&2; exit 1; }
  ...
  {
    echo "export KEEP_ADDR_${idx}=\"$addr\""
    echo "export KEEP_KEY_${idx}=\"$key\""
  } >> "$OUT_ENV"
done

chmod 600 "$OUT_ENV"
echo "[*] wrote KEEP pool env: $OUT_ENV"
